...
`cosine_similarity = dot(A.B)/(mag(A) × Mag(B))`, looks familiar? 
...
But realistically in practice embeddings models tends to produce positive vectors, cus two words in a sense would never be truly opposite there will be a sense of direction which will/can capture a similarity, say `hot` × `cold` but they can be placed in same direction of `temperature`, right?
